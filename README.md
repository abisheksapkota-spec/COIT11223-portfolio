# COIT11223, e-Portfolio

This repository contains my e-Portfolio submissions for **COIT11223** (Term 2, 2026).

## 📂 Contents

| File | Description |
|---|---|
| [`e-portfolio-1-Artificial-Intelligence.md`](./e-portfolio-1-Artificial-Intelligence.md) | e-Portfolio 1 - Artificial Intelligence: four artefacts (explainer video, news article, journal article, workshop reflection) with summaries, justifications, and workshop attendance evidence |
| `attendance-photo-1.jpeg` | Workshop attendance photo_group photo in the computer lab |
| `attendance-photo-2.jpeg` | Workshop attendance photo_lecture slide on screen |

## 👤 Student Details

- **Name:** Abishek Sapkota
- **Student number:** 12312491
- **Unit:** COIT11223
- **Term:** T2 2026

## 📖 About

This e-portfolio was developed as part of my ICT studies to reflect on and critically engage with Artificial Intelligence concepts covered across weekly workshops. Each artefact includes a summary, a personal justification for its selection, and a link to the relevant workshop discussion.

## 🔗 Quick Link

👉 [View e-Portfolio 1: Artificial Intelligence](./e-portfolio-1-Artificial-Intelligence.md)
